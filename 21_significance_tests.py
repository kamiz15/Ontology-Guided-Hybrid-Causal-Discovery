...
def wilcoxon_one_sided(x: np.ndarray, y: np.ndarray) -> tuple[float, float, str]:
    ...
    nonzero = diff[diff != 0]
    if len(nonzero) == 0:
        return float("nan"), float("nan"), "all differences zero — test not applicable"
    if len(nonzero) < 3:
        return float("nan"), float("nan"), f"only {len(nonzero)} non-zero differences — insufficient for test"
    try:
