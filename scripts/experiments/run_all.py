...
#   python scripts/experiments/run_all.py --skip-deci
#   python scripts/experiments/run_all.py --datasets synthetic_n2000,real
# ============================================================
...
def load_adapter() -> Any:
    ...
    return load_module_from_path(
        "constraint_adapter_step_14",
        ROOT / "scripts" / "constraints" / "14_constraint_adapter.py",
    )
...
def ensure_advisor_dummy_artifacts(allow_dummy_regeneration: bool) -> None:
    ...
    builder = load_module_from_path(
        "advisor_dummy_reference_builder",
        ROOT / "scripts" / "experiments" / "05_build_reference_dag_from_dummy.py",
    )
    ...
    if config["has_ground_truth"]:
        if not Path(config["ground_truth"]).exists() and dataset_name == "advisor_dummy":
            raise FileNotFoundError(
                ...
                "`.\\.venv\\Scripts\\python.exe scripts\\experiments\\05_build_reference_dag_from_dummy.py` first."
            )
...
def deci_subprocess_entry(input_path: str, output_path: str) -> int:
    ...
    try:
        ...
        deci_module = load_module_from_path(
            "step_07_run_deci",
            ROOT / "scripts" / "experiments" / "07_run_deci.py",
        )
