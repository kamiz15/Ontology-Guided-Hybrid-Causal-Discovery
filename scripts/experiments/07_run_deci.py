# 07_run_deci.py
# ============================================================
# DECI â€” Deep End-to-end Causal Inference (Microsoft Causica)
#
...
#
# Usage:
#   python scripts/experiments/07_run_deci.py
#   python scripts/experiments/07_run_deci.py --epochs 2000 --device cuda
#   python scripts/experiments/07_run_deci.py --mode constrained
# ============================================================
...
from config import FORBIDDEN_EDGES_REAL_MODULE_PATH, READY_DATA_PATH

# â”€â”€ Ontology constraints â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def _load_constraint_edges() -> tuple[list[tuple[str, str]], list[tuple[str, str]]]:
    module_path = REPO_ROOT / FORBIDDEN_EDGES_REAL_MODULE_PATH
    spec = importlib.util.spec_from_file_location("deci_forbidden_edges_real", module_path)
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load constraint module from {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return list(getattr(module, "FORBIDDEN_EDGES", [])), list(getattr(module, "REQUIRED_EDGES", []))


FORBIDDEN_EDGES, REQUIRED_EDGES = _load_constraint_edges()
...
# ================================================================
#  UTILITIES  (self-contained â€” no cross-script imports)
# ================================================================

def load_data(path: str) -> pd.DataFrame:
    """Load causal-ready dataset, keep only numeric, drop NaN rows."""
    df = pd.read_csv(path)
    print(f"[data] Loaded {path}: {df.shape[0]} rows Ã— {df.shape[1]} columns")
    ...
    print(f"[data] Dropped {before - len(numeric_df)} NaN rows â†’ {len(numeric_df)} remain")
    return numeric_df
...
def run_deci(
    ...
    print(f"\n{'=' * 60}")
    print(f"  DECI ({mode_label}) â€” Deep End-to-end Causal Inference")
    print(f"{'=' * 60}")
    ...
    # â”€â”€ If causica is not installed, provide fallback instructions â”€â”€
    ...
    # â”€â”€ Full causica DECI path â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print("  Using Microsoft Causica DECI module...")
    # [causica programmatic API would go here â€” see note below]
...
def _run_deci_manual(
    ...
    # â”€â”€ Standardize data â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Graph parameters (variational distribution) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # theta[i,j] = logit of P(edge i->j exists)
    theta = nn.Parameter(torch.randn(n_vars, n_vars, device=dev) * 0.01)

    # â”€â”€ Neural network SEM: x_j = f_j(pa(j)) + noise â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    class ANMSEM(nn.Module):
        ...
        def forward(self, x, adj):
            """
            x: (batch, n_vars)
            adj: (n_vars, n_vars) â€” soft adjacency from Gumbel-sigmoid
    ...
    # â”€â”€ Augmented Lagrangian for DAG constraint â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # h(W) = tr(e^(Wâˆ˜W)) - d = 0  iff W is a DAG
    def dag_penalty(adj):
        """NOTEARS-style DAG constraint: h(W) = tr(e^{Wâ—¦W}) - d"""
    ...
    # â”€â”€ Constraint matrix application â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    constraint_tensor = None
    if constraint_matrix is not None:
        constraint_tensor = torch.tensor(constraint_matrix, dtype=torch.float32, device=dev)

    # â”€â”€ Optimizer setup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Extract final graph â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    with torch.no_grad():
    ...
    print(f"  â†’ Saved: {gml_path}")
    print(f"  â†’ Saved: {prob_path} (continuous edge probabilities)")
...
def main():
    ...
    # â”€â”€ Unconstrained DECI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Constrained DECI (ontology-guided) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Quick comparison â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    if len(results) == 2:
