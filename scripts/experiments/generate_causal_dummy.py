...
========================
Generates ESG-Finance_dummy_data_causal_v2.csv â€” a version of the advisor
...
Compared to the original Dummy_dataset_ESG.txt:
  ...
  - Variables that have parents in the reference DAG are generated as linear
    ...
    valid range.
  - Signal-to-noise is moderate (SNR â‰ˆ 0.5â€“0.7 per parent) so algorithms
    have real structure to recover without it being trivially obvious.
  - The same data-quality problems are injected afterward: 5â€“20% missing
...
Usage:
    python scripts/experiments/generate_causal_dummy.py
    python scripts/experiments/generate_causal_dummy.py --n 3000 --seed 42 --snr 0.6 --out data/advisor_dummy
"""
...
# ---------------------------------------------------------------------------
# Variable specs â€” dtype, range, and sign of each parent coefficient
# ---------------------------------------------------------------------------
...
#   +1  = positive relationship (parent increase â†’ child increase)
#   -1  = negative relationship (parent increase â†’ child decrease)
...
TOPO_ORDER = [
    # roots â€” generated randomly, no parents in the DAG
...
def _generate_child(
    ...
    if dtype == "float":
        # Normalised latent â†’ clip to [0,1] â†’ rescale to [lo, hi]
        mid = 0.5
...
def main() -> None:
    ...
    print(f"Data CSV   : {csv_path}  ({len(df)} rows Ã— {len(df.columns)} cols)")
    print(f"Adj matrix : {adj_path}  ({adj.shape[0]} Ã— {adj.shape[1]})")
    print(f"Edge list  : {edges_path}  ({len(edge_rows)} edges)")

    # Quick sanity check: report correlation between each parent and child
    print("\nSanity check â€” Pearson r between parent and child (numeric only):")
